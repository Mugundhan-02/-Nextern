...
#
# POST /api/v1/auth/register  — create a new user account immediately
...
def register(body: RegisterRequest, db: Session = Depends(get_db)):
    ...
    # Application-level duplicate check (fast path, before hashing)
    if crud.get_user_by_email(db, body.email):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Email already registered",
        )
    # Create user — DB-level unique constraint is the final guard
    try:
    ...
    logger.info("[auth] Registered user_id=%s email=%r", user.id, user.email)
