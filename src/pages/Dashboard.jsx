...
function RecommendedInternships({ user }) {
  ...
  useEffect(() => {
    const kw = user?.specialization || user?.degree_program || 'software developer'
    const deg = user?.degree_program || 'all'
    fetch(`${API}/jobs?keyword=${encodeURIComponent(kw)}&degree=${encodeURIComponent(deg)}&page_size=4`)
